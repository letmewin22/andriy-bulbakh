...
export default class Nav {
  constructor() {
    this.store = {
      ...
      navRewealers: [
        document.querySelector('.nav__rewealer'),
        document.querySelector('.nav__rewealer-white'),
      ],
    ...
    this.store.navItems.forEach((elem) =>
      elem.addEventListener('click', this.events.bind(this)),
    )
  }
  ...
  openAnim() {
    ...
    tl.fromTo(
      this.store.navRewealers[1],
      1,
      {opacity: 0},
      {opacity: 0.8, ease: Power1.easeInOut},
    )
      .fromTo(
        this.store.navRewealers[0],
        1.5,
        {y: '-100%'},
